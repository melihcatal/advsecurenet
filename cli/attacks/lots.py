...
def _generate_target_images(config_data, all_data, labels):
    ...
    if config_data['target_images_dir']:
        try:
            target_images, target_labels = get_custom_data(
                config_data['target_images_dir'])
        except Exception as e:
            raise ValueError("Error loading target images! Details: {e}")

    elif config_data['auto_generate_target_images']:
        target_images, target_labels = generate_target_images(
            all_data, labels, config_data['maximum_generation_attempts'])
...
def execute_lots_attack(config_data):
    ...
    target_images, target_labels = _generate_target_images(
        config_data, all_data, labels)
    ...
    data_loader = DataLoaderFactory.get_dataloader(
        data, batch_size=config_data['batch_size'], shuffle=False)
    ...
    for images, labels in tqdm(data_loader, desc="Generating adversarial samples"):
        ...
        target_images_batch = target_images[total_samples:total_samples + images.size(
            0)]
        target_labels_batch = target_labels[total_samples:total_samples + images.size(
            0)]
