...
from advsecurenet.utils.model_utils import download_weights as util_download_weights, train as util_train, test as util_test, save_model
...
def train(config, model_name, dataset_name, epochs, batch_size, lr, optimizer, loss, save_path, save_name, device):
    ...
    try:
        ...
        datasetObj = DatasetFactory.load_dataset(dataset_type)
        trainData = datasetObj.load_dataset(train=True)
        trainDataLoader = DataLoaderFactory.get_dataloader(trainData, batch_size=config_data['batch_size'], shuffle=True)

        model = ModelFactory.get_model(config_data['model_name'], num_classes=datasetObj.num_classes)
        model.train()

        util_train(model, trainDataLoader, epochs=config_data['epochs'], learning_rate=config_data['lr'], device=device)
