...
@pytest.mark.cli
@pytest.mark.essential
@patch("cli.shared.utils.config.read_yml_file", return_value={"key": "value", "override_key": "override_initial_value"})
...
def test_attack_config_check():
    ...
    with pytest.raises(ValueError, match="Please set dataset-name to 'custom' when specifying custom-data-dir."):
        attack_config_check({'dataset_name': 'non_custom',
                            'custom_data_dir': 'some_dir'})


@pytest.mark.cli
@pytest.mark.essential
@patch("cli.shared.utils.config.click.secho")
@patch("cli.shared.utils.config.get_default_config_yml", return_value="default_config.yml")
@patch("cli.shared.utils.config.load_configuration", return_value={"key": "value"})
@patch("cli.shared.utils.config.recursive_dataclass_instantiation", return_value=SampleConfig(field1=1, field2="test"))
def test_load_and_instantiate_config(mock_recursive_instantiation, mock_load_configuration, mock_get_default_config_yml, mock_click_secho):
    config = ""
    ...
    mock_recursive_instantiation.assert_called_once_with(
...
def test_generate_default_config_yaml(mock_yaml, mock_open, mock_read_yml_file, mock_makedirs, mock_path_exists):
    ...
    mock_yaml_instance.dump.assert_called_once_with(
        {"key": "value"}, mock_open())
...
@pytest.mark.essential
@patch("cli.shared.utils.config.os.path.exists", return_value=True)
@patch("cli.shared.utils.config.os.walk", return_value=[("dirpath", None, ["test_config.yml"])])
