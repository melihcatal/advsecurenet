...
from cli.shared.utils.helpers import (
    get_device_from_cfg,
    read_data_from_file,
    save_images,
    to_bchw_format,
...
def test_save_images(
    mock_tqdm, mock_path_join, mock_open, mock_to_pil_image, mock_makedirs
):
...
def test_read_data_from_file_text_tensor_str(mock_open):
    ...
    with pytest.raises(ValueError):
        read_data_from_file(
            "test.txt", cast_type=str, return_type=torch.Tensor, separator="\n"
        )
...
def test_read_data_from_file_text_invalid_return_type(mock_open):
    with pytest.raises(ValueError) as exc_info:
        read_data_from_file("test.txt", cast_type=str, return_type=int, separator="\n")
