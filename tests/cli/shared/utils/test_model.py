...
from cli.shared.types.utils.model import (
    ModelCliConfigType,
    ModelNormConfig,
    ModelPathConfig,
)
...
def mock_config():
    return ModelCliConfigType(
        ...
        path_configs=ModelPathConfig(
            model_arch_path="path/to/arch", model_weights_path="path/to/weights"
        ),
...
def test_create_model_load_weights(mock_torch_load, mock_click_secho, mock_config):
    ...
    with patch(
        "cli.shared.utils.model.ModelFactory.create_model", return_value=mock_model
    ):
...
def test_create_model_add_norm_layer(
    mock_NormalizationLayer, mock_create_model, mock_config
):
...
def test_validate_norm_layer_missing_mean_or_std():
    ...
    with pytest.raises(
        ValueError,
        match=CLIErrorMessages.TRAINER.value.NORM_LAYER_MISSING_MEAN_OR_STD.value,
    ):
...
def test_validate_norm_layer_invalid_type_mean_and_std(mock_config):
    mock_config.norm_config.add_norm_layer = True
    mock_config.norm_config.norm_mean = torch.randn(mock_config.num_input_channels)
    mock_config.norm_config.norm_std = torch.randn(mock_config.num_input_channels)
    with pytest.raises(
        ValueError,
        match=CLIErrorMessages.TRAINER.value.NORM_LAYER_MEAN_OR_STD_NOT_LIST.value,
    ):
