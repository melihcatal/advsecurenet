...
from cli.logic.utils.model import (
    _get_models,
...
@patch("click.echo")
@patch(
    "advsecurenet.models.model_factory.ModelFactory.available_models",
...
@patch("click.echo")
@patch(
    "advsecurenet.models.standard_model.StandardModel.available_weights",
    return_value=[ResNet18_Weights.IMAGENET1K_V1],
)
def test_cli_available_weights(mock_available_weights, mock_echo):
    ...
    mock_echo.assert_any_call("Available weights for resnet18:")
...
def test_cli_model_layers_no_normalization(mock_create_model, mock_echo, mock_secho):
    ...
    mock_secho.assert_called_once_with("Layers of model1:", bold=True, fg="green")
...
def test_cli_model_layer_with_normalization(mock_create_model, mock_echo, mock_secho):
    mock_model = MagicMock()
    mock_model.get_layer_names.return_value = ["NormalizationLayer", "layer1", "layer2"]
    ...
    mock_secho.assert_called_once_with("Layers of model1:", bold=True, fg="green")
    mock_echo.assert_any_call(f"{'Layer Name':<30}{'Layer Type':<30}")
    mock_echo.assert_any_call(f"{'NormalizationLayer':<30}{'NormalizationLayer':<30}")
    mock_echo.assert_any_call(f"{'layer1':<30}{'Conv2D':<30}")
    mock_echo.assert_any_call(f"{'layer2':<30}{'Conv2D':<30}")
...
def test_cli_download_weights_file_exists_error(mock_download_weights, capsys):
    with pytest.raises(click.ClickException) as excinfo:
        cli_download_weights("model1", "dataset1", "filename", "save_path")
    assert (
        str(excinfo.value) == "File filename already exists in the specified directory!"
    )
...
def test_cli_download_weights_http_error(mock_download_weights, capsys):
    ...
    assert (
        str(excinfo.value)
        == "Could not download weights for model1 trained on dataset1!"
    )
...
def test_get_models_custom():
    with patch(
        "advsecurenet.models.model_factory.ModelFactory.available_custom_models",
        return_value=["custom_model1", "custom_model2"],
    ) as mock_available_custom_models:
...
def test_get_models_standard():
    with patch(
        "advsecurenet.models.model_factory.ModelFactory.available_standard_models",
        return_value=["standard_model1", "standard_model2"],
    ) as mock_available_standard_models:
