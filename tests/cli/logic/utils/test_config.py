...
@patch("click.echo")
@patch(
    "cli.logic.utils.config.generate_default_config_yaml", side_effect=FileNotFoundError
)
def test_cli_config_default_file_not_found(
    mock_generate_default_config_yaml, mock_echo, mock_secho
):
    with patch("builtins.print") as mock_print:
        ...
        mock_echo.assert_called_once_with(
            "Configuration file config1 not found! You can use the 'configs' command to list available configuration files.",
            err=True,
        )
...
def test_cli_config_default_save(
    mock_generate_default_config_yaml, mock_echo, mock_secho
):
    cli_config_default("config1", save=True)
    ...
    mock_echo.assert_called_once_with("Generated default configuration file config1!")
...
@patch("click.echo")
@patch("cli.logic.utils.config.generate_default_config_yaml")
def test_cli_config_default_print(
    mock_generate_default_config_yaml, mock_echo, mock_secho
):
...
@patch("click.echo")
@patch(
    "cli.logic.utils.config.generate_default_config_yaml",
    side_effect=Exception("General error"),
)
def test_cli_config_default_general_error(
    mock_generate_default_config_yaml, mock_echo, mock_secho
):
    with patch("builtins.print") as mock_print:
        cli_config_default("config1", save=True)
        ...
        mock_echo.assert_called_once_with(
            "Error generating default configuration file config1! Details: General error",
            err=True,
        )
