...
def test_cli_adversarial_evaluation(
    mock_CLIAdversarialEvaluator, mock_load_and_instantiate_config
):
...
def test_cli_list_adversarial_evaluations(mock_secho):
    with patch(
        "cli.logic.evaluation.evaluation.adversarial_evaluators",
        {"eval1": MagicMock(), "eval2": MagicMock()},
    ):
        cli_list_adversarial_evaluations()
