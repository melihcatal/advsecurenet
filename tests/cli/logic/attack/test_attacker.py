...
def attacker_config(device):
    device_config = DeviceConfig(processor=device, use_ddp=False)
...
def ddp_attacker(mock_prepare_dataset, attacker_config):
    with mock.patch("torch.distributed.init_process_group"), mock.patch(
    ...
    ), mock.patch("torch.distributed.get_rank", return_value=0), mock.patch(
        ...
        "torch.nn.parallel.DistributedDataParallel", autospec=True
    ):
        rank = 0
        world_size = 2
...
@patch("cli.logic.attack.attacker.CLIAttacker._prepare_dataset")
def test_cli_attacker_execute_single_gpu(
...
def test_cli_attacker_execute_multi_gpus(
    mock_execute_ddp_attack, mock_prepare_dataset, mock_adv_target_gen, attacker_config
):
    # attacker
    ...
    with mock.patch.object(
        attacker, "_execute_ddp_attack"
    ) as mock_execute_ddp_attack, mock.patch.object(logger, "info") as mock_logger_info:
        attacker.execute()
...
@patch(
    "cli.logic.attack.attacker.DDPAttacker.gather_results", return_value=["dummy_image"]
)
...
def test_cli_attacker_execute_ddp_attack(
    ...
    mock_ddp_coordinator.assert_called_once_with(ddp_attacker._ddp_attack_fn, 2)
...
def test_cli_attacker_get_target_labels(mock_prepare_dataset, attacker_config):
    ...
    attacker_config.attack_config.target_parameters.target_labels_path = (
        "path/to/labels"
    )
    ...
    with patch(
        "cli.logic.attack.attacker.read_data_from_file", return_value=[1, 2, 3]
    ) as mock_read_data:
...
def test_cli_attacker_get_target_labels_not_targeted(
    mock_prepare_dataset, attacker_config
):
    attacker_config.attack_config.target_parameters.targeted = False
    attacker = CLIAttacker(attacker_config, AttackType.FGSM)
...
def test_cli_attacker_validate_dataset_availability(
    ...
    assert (
        attacker._validate_dataset_availability(valid_dataset, "train") == valid_dataset
    )

    with pytest.raises(
        ValueError,
        match="The dataset part 'test' is specified but no test data is available.",
    ):
...
def test_cli_attacker_sample_data(
    mock_random_split, mock_prepare_dataset, attacker_config
):
...
def test_sample_data_if_required_no_sampling(
    mock_validate_dataset, mock_prepare_dataset, attacker_config, sample_size
):
...
def test_sample_data_if_required(
    mock_sample_data,
...
@patch("cli.logic.attack.attacker.CLIAttacker._prepare_dataset")
@patch(
    "cli.logic.attack.attacker.AdversarialTargetGenerator.generate_target_images_and_labels",
    return_value=([torch.randn(1, 3, 32, 32)], torch.tensor([1])),
)
def test_generate_target_lots(
    mock_generate_target_images, mock_prepare_dataset, attacker_config
):
...
@patch("cli.logic.attack.attacker.CLIAttacker._prepare_dataset")
@patch(
    "cli.logic.attack.attacker.AdversarialTargetGenerator.generate_target_labels",
    return_value=torch.tensor([1]),
)
def test_generate_target_fgsm(
    mock_generate_target_labels, mock_prepare_dataset, attacker_config
):
    # Create the CLIAttacker instance
    attacker = CLIAttacker(attacker_config, AttackType.FGSM)
    ...
    # Check if generate_target_images_and_labels was called once with the correct data
    mock_generate_target_labels.assert_called_once_with(data=data, overwrite=True)
...
def test_generate_target_labels_auto_generate(
    mock_adversarial_dataset, mock_prepare_dataset, attacker_config
):
    attacker_config.attack_config.target_parameters.targeted = True
    ...
    with patch.object(
        attacker, "_generate_target", return_value=all_data
    ) as mock_generate_target:
...
@patch("cli.logic.attack.attacker.CLIAttacker._prepare_dataset")
@patch("cli.logic.attack.attacker.AdversarialDataset")
def test_generate_target_labels_with_labels(
    mock_adversarial_dataset, mock_prepare_dataset, attacker
):
    all_data = MagicMock()
...
@patch("cli.logic.attack.attacker.CLIAttacker._prepare_dataset")
@patch("cli.logic.attack.attacker.AdversarialDataset")
def test_generate_target_labels_no_target_parameters(
    mock_adversarial_dataset, mock_prepare_dataset, attacker
):
    all_data = MagicMock()
...
@patch("cli.logic.attack.attacker.CLIAttacker._prepare_dataset")
@patch("cli.logic.attack.attacker.AdversarialDataset")
def test_generate_target_labels_not_targeted(
    mock_adversarial_dataset, mock_prepare_dataset, attacker
):
    all_data = MagicMock()
...
@pytest.mark.essential
@patch(
    "cli.logic.attack.attacker.get_datasets", return_value=(MagicMock(), MagicMock())
)
def test_prepare_dataset(mock_get_datasets, attacker):

    with patch.object(
        attacker, "_get_target_parameters", return_value=MagicMock()
    ) as mock_get_target_parameters, patch.object(
        attacker, "_get_target_labels_if_available", return_value=MagicMock()
    ) as mock_get_target_labels_if_available, patch.object(
        attacker, "_select_data_partition", return_value=MagicMock()
    ) as mock_select_data_partition, patch.object(
        attacker, "_sample_data_if_required", return_value=MagicMock()
    ) as mock_sample_data_if_required, patch.object(
        attacker, "_generate_or_assign_target_labels", return_value=MagicMock()
    ) as mock_generate_or_assign_target_labels:
        ...
        mock_generate_or_assign_target_labels.assert_called_once_with(
            mock_sample_data_if_required.return_value,
            mock_get_target_parameters.return_value,
            mock_get_target_labels_if_available.return_value,
        )
...
def test_get_target_labels_if_available_with_labels(attacker):
    ...
    with patch.object(
        attacker, "_get_target_labels", return_value=MagicMock()
    ) as mock_get_target_labels:
        target_labels = attacker._get_target_labels_if_available(target_parameters)
...
@patch("cli.logic.attack.attacker.DDPAttacker")
def test_ddp_attack_fn(
    mock_ddp_attacker, mock_prepare_attack_config, mock_init, attacker
):
...
def test_prepare_attack_config(
    mock_create_dataloader_config,
    mock_create_attack,
    mock_create_model,
    attacker,
    attacker_config,
