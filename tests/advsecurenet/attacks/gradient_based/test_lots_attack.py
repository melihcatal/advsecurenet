...
def test_attack_single_distributed(
    mock_lots_single, mock_config, mock_model, mock_tensors
):
...
def test_adversarial_success_with_target_classes(mock_config, mock_model, mock_attack):
    ...
    x_target_deep_features = torch.randn(5, 100, device=mock_config.device.processor)
    y = torch.tensor(
        [0, 1, 2, 3, 4], dtype=torch.long, device=mock_config.device.processor
    )
...
@pytest.mark.advsecurenet
@pytest.mark.essential
def test_adversarial_success_without_target_classes(
    mock_config, mock_model, mock_attack
):
    x = torch.randn(5, 3, 32, 32, device=mock_config.device.processor)
    x_deep_features = torch.randn(5, 100, device=mock_config.device.processor)
    x_target_deep_features = torch.randn(5, 100, device=mock_config.device.processor)
...
def test_validate_config_invalid_type(mock_attack):
    ...
    with pytest.raises(
        ValueError,
        match="Invalid config type provided. Expected LotsAttackConfig. But got: <class 'str'>",
    ):
        mock_attack._validate_config(config)
...
def test_validate_config_invalid_mode(mock_config, mock_attack):
    ...
    with pytest.raises(
        ValueError,
        match="Invalid mode type provided. Allowed modes are: iterative, single",
    ):
...
def test_validate_config_negative_epsilon(mock_config, mock_attack):
    ...
    with pytest.raises(
        ValueError,
        match="Invalid epsilon value provided. Epsilon must be greater than 0.",
    ):
        mock_attack._validate_config(mock_config)
...
def test_validate_config_negative_learning_rate(mock_config, mock_attack):
    ...
    with pytest.raises(
        ValueError,
        match="Invalid learning rate value provided. Learning rate must be greater than 0.",
    ):
        mock_attack._validate_config(mock_config)
...
def test_validate_config_negative_max_iterations(mock_config, mock_attack):
    ...
    with pytest.raises(
        ValueError,
        match="Invalid max iterations value provided. Max iterations must be greater than 0.",
    ):
        mock_attack._validate_config(mock_config)
...
def test_validate_config_missing_deep_feature_layer(mock_config, mock_attack):
    ...
    with pytest.raises(
        ValueError,
        match="Deep feature layer that you want to use for the attack must be provided.",
    ):
        mock_attack._validate_config(mock_config)
...
def test_lots_iterative_early_success(mock_config, mock_attack, mock_model):
    ...
    def mock_evaluate_adversarial_success(
...
def test_validate_layer_not_exists(mock_model, mock_config):
    ...
    with pytest.raises(ValueError) as excinfo:
        attack = LOTS(mock_config)
        attack._validate_layer(mock_model)
    assert f"Layer '{layer_name}' not found in the model." in str(excinfo.value)
