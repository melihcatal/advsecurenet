...
def test_adversarial_attack_init():
    device_config = DeviceConfig(processor=torch.device("cpu"), use_ddp=False)
    attack_config = AttackConfig(device=device_config, targeted=False)
    adversarial_attack = TestAdversarialAttack(attack_config)
    assert adversarial_attack.device_manager.initial_device == torch.device("cpu")
