...
def mock_dataset_with_targets():
    ...
    data = [(torch.randn(3, 32, 32), torch.tensor(i % 10)) for i in range(100)]
...
def test_save_images(mock_save, mock_dataset, target_generator):
    ...
    assert mock_save.call_count == len(paired_images) * 2  # Each pair saves two images
...
def test_generate_target_images_and_labels_success(
    mock_shuffle_and_pair, mock_generate_class_map, mock_dataset, target_generator
):
...
@patch.object(
    ...
    "_shuffle_and_pair_images_across_classes",
    return_value=[],
)
def test_generate_target_images_and_labels_empty_pairs(
    mock_shuffle_and_pair, mock_generate_class_map, mock_dataset, target_generator
):
    mock_generate_class_map.return_value = {
        i: list(range(i * 10, (i + 1) * 10)) for i in range(10)
...
@pytest.mark.advsecurenet
@pytest.mark.essential
def test_extract_targets_wrapped_with_targets(
    mock_wrapped_dataset_with_targets, target_generator
):
    targets = target_generator._extract_targets(mock_wrapped_dataset_with_targets)
    assert targets == [i % 10 for i in range(100)]


@pytest.mark.advsecurenet
@pytest.mark.essential
def test_extract_targets_wrapped_with_labels(
    mock_wrapped_dataset_with_labels, target_generator
):
    targets = target_generator._extract_targets(mock_wrapped_dataset_with_labels)
    assert targets == [i % 10 for i in range(100)]


@pytest.mark.advsecurenet
@pytest.mark.essential
def test_extract_targets_without_targets_or_labels(
    mock_dataset_without_targets_or_labels, target_generator
):
    targets = target_generator._extract_targets(mock_dataset_without_targets_or_labels)
...
@pytest.mark.essential
@patch.object(AdversarialTargetGenerator, "_load_mappings")
def test_get_existing_indices_map_invalid_data_type(
    mock_load_mappings, target_generator
):
...
def test_get_existing_indices_map_load_failure(
    mock_logger, mock_load_mappings, target_generator
):
    data_type = "imagenet"
