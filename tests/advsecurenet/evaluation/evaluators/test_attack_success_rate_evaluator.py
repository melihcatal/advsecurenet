...
@pytest.mark.essential
@patch(
    "advsecurenet.evaluation.evaluators.attack_success_rate_evaluator.AttackSuccessRateEvaluator._predict"
)
def test_update_untargeted_attack(mock_predict, evaluator, mock_model):
    ...
    evaluator.update(
        mock_model, original_images, true_labels, adversarial_images, is_targeted=False
    )
...
def test_get_correct_predictions_mask(evaluator):
    ...
    mask = evaluator._get_correct_predictions_mask(prediction_labels, true_labels)
...
def test_filter_correct_predictions(evaluator):
    ...
    correct_true_labels, correct_adversarial_images = (
        evaluator._filter_correct_predictions(mask, true_labels, adversarial_images)
    )
