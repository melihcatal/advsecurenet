...
def ddp_training_coordinator(mock_set_start_method, mock_train_func):
    return DDPCoordinator(ddp_func=mock_train_func, world_size=2)
...
def test_ddp_setup(mock_init_process_group, ddp_training_coordinator):
    ...
    mock_init_process_group.assert_called_once_with(
        backend="nccl", rank=rank, world_size=ddp_training_coordinator.world_size
    )
...
def test_run_process(
    ...
    mock_train_func.assert_called_once_with(
        rank,
        ddp_training_coordinator.world_size,
        *ddp_training_coordinator.args,
        **ddp_training_coordinator.kwargs
    )
